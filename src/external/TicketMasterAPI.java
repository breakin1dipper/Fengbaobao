...
public class TicketMasterAPI {
	...
	private static final String API_KEY = "n0EJGycd91SwHLFjFWKjfaoEddjPqrUq";
	...
	private String getImageUrl(JSONObject event) throws JSONException {
		if (!event.isNull("images")) {
			...
			for (int i = 0; i < array.length(); ++i) {
				...
				if (!image.isNull("url")) {
					String res = image.getString("url");
					return res;
				}
			}
		}
	...
	private Set<String> getCategories(JSONObject event) throws JSONException {
		...
		if (!event.isNull("classifications")) {
			JSONArray classy = event.getJSONArray("classifications");
			for (int i = 0; i < classy.length(); i++) {
				JSONObject a = classy.getJSONObject(i);
				if (!a.isNull("segment")) {
					JSONObject b = a.getJSONObject("segment");
					if (!b.isNull("name")) {
						String res = b.getString("name");
						categories.add(res);
					}
				}
			}
